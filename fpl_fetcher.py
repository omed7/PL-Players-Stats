    ...
    for player in elements:
        ...
        try:
            ...
            def calc_stats(match_list):
                def sum_stat(key, cast_type=float):
                    return sum(cast_type(m.get(key, 0)) for m in match_list)

                mins = sum_stat("minutes", int)
                ...
                return {
                    "minutes": mins,
                    "min_pct": min_pct,
                    "xG": sum_stat("expected_goals"),
                    "xA": sum_stat("expected_assists"),
                    "xGI": sum_stat("expected_goal_involvements"),
                    "xGC": sum_stat("expected_goals_conceded"),
                    "creativity": sum_stat("creativity"),
                    "threat": sum_stat("threat"),
                    "ict": sum_stat("ict_index"),
                    "bps": sum_stat("bps", int),
                    "bonus": sum_stat("bonus", int),
                    "points": sum_stat("total_points", int),
                    "saves": sum_stat("saves", int),
                    "defcon": defcon,
                }
