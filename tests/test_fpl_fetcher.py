...
class TestGetFplData(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_history_watkins = {
            'history': [
                {'minutes': 90, 'total_points': 2, 'expected_goals': 0.2}
            ]
        }
    ...
    def test_get_fpl_data_success(self, mock_file, mock_json_dump, mock_sleep, mock_get):
        ...
        mock_get.side_effect = [mock_resp_bootstrap, mock_resp_saka, mock_resp_watkins]
    ...
    @patch('fpl_fetcher.json.dump')
    @patch('builtins.open', new_callable=mock_open)
    def test_get_fpl_data_empty_history(self, mock_file, mock_json_dump, mock_sleep, mock_get):
        ...
        mock_get.side_effect = [mock_resp_bootstrap, mock_resp_saka, mock_resp_watkins_empty]
    ...
    @patch('fpl_fetcher.json.dump')
    @patch('builtins.open', new_callable=mock_open)
    def test_get_fpl_data_api_error(self, mock_file, mock_json_dump, mock_sleep, mock_get):
        ...
        mock_get.side_effect = [mock_resp_bootstrap, mock_resp_saka, Exception("API Down")]
