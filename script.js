...
function renderTable() {
  ...
  playersToRender.forEach((player) => {
    ...
    tdName.className = `player-name-cell sticky-col ${textColorClass}`;
    if (bgColor) {
      tdName.style.setProperty("background-color", bgColor, "important");
    }
    ...
    teamImg.src = player.logo;
    teamImg.alt = `${player.team} logo`;
    teamImg.className = "team-logo";
    teamImg.style.marginRight = "0";
    teamLogoContainer.appendChild(teamImg);
    ...
    wrapper.appendChild(teamLogoContainer);

    const detailsDiv = document.createElement("div");
    detailsDiv.className = "player-details";
    const nameDiv = document.createElement("div");
    nameDiv.className = "player-name-text";
    nameDiv.textContent = player.name;
    detailsDiv.appendChild(nameDiv);
    wrapper.appendChild(detailsDiv);

    const minutesDiv = document.createElement("div");
    minutesDiv.className = "player-minutes-corner";
    minutesDiv.textContent = `Mins:${minutes} %${min_pct}`;
    wrapper.appendChild(minutesDiv);
    ...
    tr.appendChild(
      createCell(
        saves,
        `col-saves ${state.positionFilter === "GK" ? "" : "hidden"}`,
      ),
    );
    tr.appendChild(
      createCell(
        defcon,
        `col-defcon ${state.positionFilter === "GK" ? "hidden" : ""}`,
      ),
    );
    tr.appendChild(
      createCell(xG, `col-xg ${state.positionFilter === "GK" ? "hidden" : ""}`),
    );
    tr.appendChild(
      createCell(xA, `col-xa ${state.positionFilter === "GK" ? "hidden" : ""}`),
    );
    tr.appendChild(
      createCell(
        xGI,
        `col-xgi ${state.positionFilter === "GK" ? "hidden" : ""}`,
      ),
    );
    tr.appendChild(createCell(xGC));
    tr.appendChild(
      createCell(
        creativity,
        `col-creativity ${state.positionFilter === "GK" ? "hidden" : ""}`,
      ),
    );
    tr.appendChild(
      createCell(
        threat,
        `col-threat ${state.positionFilter === "GK" ? "hidden" : ""}`,
      ),
    );
    tr.appendChild(
      createCell(
        ict,
        `col-ict ${state.positionFilter === "GK" ? "hidden" : ""}`,
      ),
    );
    tr.appendChild(createCell(bps));
    tr.appendChild(createCell(bonus));
